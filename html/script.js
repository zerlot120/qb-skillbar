$(document).ready(function(){
        ...
        $(".bar-container").fadeIn('fast', function() {
            $(".bar-total").stop().css({"width": 0}).animate({
              ...
              complete: function() {
                ...
                $(".bar-check").css({
                    "background-color": "rgba(255, 69, 2, 0.897)"
                });
                $(".bar-total").stop();
                // setTimeout(function(){
                    $(".bar-container").fadeOut('fast', function() {
                        $(".bar-total").css("left", 0, "width", 0);
                        $(".bar-check").css({"background-color": "rgba(92, 255, 39, 0.897)"});
    ...
    Skillbar.Check = function(data) {
        ...
        if (Percentage + 2 >= Minimum && Percentage - 2 <= Check) {
            $(".bar-check").css({
                "background": "rgba(159, 255, 120, 0.917)",
		"transition": "background-color .1s ease"
            ...
            $(".bar-check").css({
                "background": "rgba(92, 255, 39, 0.917)"
            });
            $.post('https://qb-skillbar/Check', JSON.stringify({
                success: false
            }));
        ...
        $(".bar-container").fadeOut('fast', function() {
            $(".bar-total").css("left", 0);
            $(".bar-check").css({"background": "rgba(255,69,2,0.34)"});
        });
    }
